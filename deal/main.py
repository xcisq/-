...
from utils1 import load_current_data, simulate_with_given_current, visualize_and_save_on_map
import random
D = 600  # 扩散系数，单位是平方米每秒 (m²/s)
...
for day in range(1, frames + 1):
    ...
    u_current, v_current = load_current_data(nc_file)  # 加载当天的洋流数据
    # 模拟扩散
    concentration = simulate_with_given_current(concentration, D, dt, dx, decay_constant, v_current, u_current, beta,index)
    index=index+1
    # 可视化并保存结果到地图上
    visualize_and_save_on_map(concentration, day, output_dir, extent)
    # 打印状态更新
