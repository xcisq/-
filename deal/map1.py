...
# ax.set_extent([120, 180, 0, 60], crs=ccrs.PlateCarree())
#
# # 添加陆地特征，填充为灰色
...
#
# # 设置图形尺寸
# fig = plt.figure(figsize=(6, 6))  # 6x6英寸
#
# # 创建地图投影
# ax = plt.axes(projection=ccrs.PlateCarree())
#
...
# ax.gridlines(draw_labels=True, linewidth=0.5, color='black', alpha=0.5, linestyle='--')
#
# # 保存图像到文件，指定DPI来达到600x600像素的分辨率
# # DPI（每英寸点数）是100，因此6x6英寸的图像在100 DPI时为600x600像素
# plt.savefig('map_600x600.png', dpi=100)
# plt.close(fig)
#
# #提示保存位置
# 'map_600x600.png'
...
# 设置图形尺寸
fig = plt.figure(figsize=(6, 6), dpi=100)  # 6x6英寸，100DPI
...
plt.savefig('map_600x600.png', bbox_inches='tight', pad_inches=0)
plt.close(fig)
