...
function startDrawing(data, mode) {
    ...
    const stops_3DT = [
        ...
        { position: 0.932, color: '#FFFF00' },
        { position: 0.950, color: '#FFC000' },
        { position: 0.966, color: '#FF8100' },
        { position: 0.983, color: '#FF4200' },
        { position: 1.00, color: '#FF0000' }
    ];
    // 三维盐度初始colorbar
    const stops_3DS = [
        { position: 0.00, color: '#0000FF' },
        { position: 0.625, color: '#66E0FF' },
        { position: 0.70, color: '#80FF5E' },
        { position: 0.825, color: '#FFFF33' },
        { position: 0.95, color: '#FF3200' },
        { position: 1.00, color: '#FF0000' }
    ];
    ...
    // const to2D = document.getElementById("to2D");
    // to2D.addEventListener('click', () => {
    //     drawProfile(latSlice, lonSlice);
    // })

    
    // 温度/盐度随深度变化剖面数据的获取
    function drawProfile(latSlice, lonSlice) {
        for (let j = 0; j < shape[0]; j++) {
            const selectedValue = data[lonSlice + (latSlice * shape[2]) + j * shape[1] * shape[2]];
            slice_profile.push(selectedValue);
        }
        console.log(`kanzheli: ${slice_profile}`);

        dataProfile = depList.map(function (depth, index) {
            return { depth: depth, value: slice_profile[index] };
        });
        // console.log(depList);
        // console.log(slice);
        // console.log(dataProfile);
